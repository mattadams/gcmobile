 ...
 * desired icon and the background changes from black to orange.
 * 
 * This is almost identical to GridWidget, except multiple icaons can be selected simultaneously. 
 * 
...
public class GridMultiWidget extends QuestionWidget {
        ...
        public GridMultiWidget(Context context, FormEntryPrompt prompt, int numColumns) {
                ...
                gridview.setOnItemClickListener(new OnItemClickListener() {
                        public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                                ...
                                if (selected[position]){
                                        ...
                                        imageViews[position].setBackgroundColor(Color.rgb(orangeRedVal,orangeGreenVal,orangeBlueVal));
                                }

                        }
                });

                //Read the screen dimensions and fit the grid view to them. It is important that the grid view
                ...
                int screenWidth = display.getWidth();
                int screenHeight = display.getHeight();
                GridView.LayoutParams params = new GridView.LayoutParams(screenWidth-15, screenHeight-10);
                ...
                for ( int i = 0 ; i < choices.length; ++i ) {
                        ...
                        selected[i] = found;
                        if(selected[i]){
                                imageViews[i].setBackgroundColor(Color.rgb(orangeRedVal,orangeGreenVal,orangeBlueVal));
                        } else{
