...
/**
 * AutoCompleteWidget handles select-one fields using an autocomplete text box.
 * The user types part of the desired selection and suggestions appear in a list 
 * below. The full list of possible answers is not displayed to the user. The goal
 * is to be more compact; this question type is best suited for select one questions
 * with a large number of possible answers. 
 * 
...
public class AutoCompleteWidget extends QuestionWidget{
        ...
        public IAnswerData getAnswer() {
                ...
                //If the user has typed text into the autocomplete box that doesn't match any answer, warn them that their
                //solution didn't count. 
                if (!response.equals("")) {
                        Toast.makeText(getContext(), "Warning: \"" + response + "\" does not match any answers. No answer recorded.", Toast.LENGTH_LONG).show();
                }
                return null;
        }
                        ...
                        protected FilterResults performFiltering(CharSequence prefix) {
                                ...
                                if (prefix == null || prefix.length() == 0) {
                                        ...
                                        for (int i = 0; i < count; i++) {
                                                ...
                                                if (filterType.equals(match_substring) && (item_compare.startsWith(prefixString) || item_compare.contains(prefixString))) {
                                                        newItems.add(item);
                                                } else if(filterType.equals(match_prefix) && item_compare.startsWith(prefixString)){ 
                                                        newItems.add(item);
                                                        ...
                                                        if (item_compare.startsWith(prefixString) || item_compare.contains(prefixString)){
                                                                newItems.add(item);
                                                        }
                                                } 
                                        }
