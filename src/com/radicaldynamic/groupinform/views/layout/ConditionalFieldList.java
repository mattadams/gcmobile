...
import com.radicaldynamic.groupinform.widgets.IMultipartSelectWidget;
...
public class ConditionalFieldList implements IGroupLayout {
    ...
    /**
     * Return map of ( TreeReference : String ) that contains the TreeReference values of all the
     * form widgets that have a simple relevancy condition involving the given select widget. The
     * String associated with the TreeReference is the item value of the select widget for which
     * they are relevant. A simple relevancy condition is a binding of the form, e.g.: <bind
     * nodeset="/widgets/int_a" relevant="selected(/widgets/select, 'a')" /> I.e., the
     * relevant="..." expression is a selected() predicate whose second argument is a single string
     * literal that corresponds to a selection value. If a TreeReference is relevant for two or more
     * values, it will not appear in the map.
     * 
    ...
    private Map<TreeReference, String> findSubfieldPairings(FormIndex fi) {
        ...
        // )
        // such that the tree references can be then correlated to a form index of the subsequent
        // controls.
        ...
        // the vector of computations that depend upon the value of this form index are maintained
        // in the
        // triggerIndex hash. This has explicit dependencies but on implicit (nested) dependencies.
        // For
        // example, an entire group could have a relevant="..." binding; in that case, the elements
        // within
        ...
        if (o == null || !(o instanceof Vector))
            return treeRefValueMap; // just to keep everyone honest...
                ...
                if ((c.trueAction == Condition.ACTION_SHOW) && (c.expr instanceof XPathConditional)) {
                    ...
                    if (xe.getExpr() instanceof XPathFuncExpr) {
                        ...
                        if ("selected".equals(xfe.id.toString())) {
                            ...
                            // assume the LHS is the selectOne or selectMulti referenced by the fi,
                            // the FormIndex.
    ...
    public void buildView(GroupView view, List<FormIndex> indices, File instanceDir,
        ...
        AbstractQuestionWidget qvSelect =
        ...
        if (values != null && qvSelect instanceof IMultipartSelectWidget) {
                    ...
                    for (FormIndex nestedFormIndex : nestedWidgets) {
                        formEntryPrompt =
                            Collect.getInstance().getFormEntryController().getModel()
                                    .getQuestionPrompt(nestedFormIndex);
                        AbstractQuestionWidget nestedWidget =
                            WidgetFactory.createWidgetFromPrompt(view.getHandler(),
                                formEntryPrompt, view.getContext(), instanceDir);
            ...
            Log.e(getClass().getName(),
                        "Partial view construction attempted on non-multipart-build widget or one with no dependencies");
